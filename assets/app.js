...
(function () {
      ...
      // 14. 导出 Word (.docx) 文档 (双页带完整地图截图，与 PDF 导出效果一致)
      document.getElementById('btn-export-docx').addEventListener('click', async function () {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-1"></i>生成中...';
        showToast('正在生成双页标准 Word 文档（含地图截图）...');

        try {
          // 1. 打开预览模态框，确保两页渲染并生成地图快照
          openPreviewModal();
          const p1 = document.getElementById('print-page-1');
          const p2 = document.getElementById('print-page-2');
          if (!p1 || !p2) { alert('预览容器未初始化，请重试'); return; }
          p1.classList.remove('hidden');
          p2.classList.remove('hidden');
          ...
          const toArrayBuffer = (canvas) => new Promise((resolve, reject) => {
            canvas.toBlob((blob) => {
              if (!blob) return reject(new Error('canvas 转图片失败'));
              const reader = new FileReader();
          ...
          const boldCenter = (text) => new Paragraph({
            children: [new TextRun({ text: text, bold: true })],
            alignment: AlignmentType.CENTER
          });

          // 5. 第1页表格：左基本信息 + 右水源图(嵌入截图)
          const tableRows1 = [
            new TableRow({
              children: [
                new TableCell({
                  ...
                  children: [boldCenter('基 本 信 息')]
                }),
                new TableCell({
                  width: { size: 55, type: WidthType.PERCENTAGE },
                  children: [boldCenter('水 源 图')]
                })
              ]
            })
          ];
          ...
          fields.forEach((f, idx) => {
            ...
            if (idx === 0) {
              tableRows1.push(
                new TableRow({
                  children: [
                    new TableCell({ width: { size: 12, type: WidthType.PERCENTAGE }, children: [new Paragraph({ children: [new TextRun({ text: f.label, bold: true })] })] }),
                    new TableCell({ width: { size: 33, type: WidthType.PERCENTAGE }, children: [new Paragraph({ text: val })] }),
                    new TableCell({
                      width: { size: 55, type: WidthType.PERCENTAGE },
                      children: [new Paragraph({ children: [new ImageRun({ type: 'png', data: imgBuf1, transformation: { width: fit1.width, height: fit1.height } })], alignment: AlignmentType.CENTER })]
                    })
                  ]
                })
              );
          ...
          const tableRows2 = [
            ...
            new TableRow({
              children: [
                new TableCell({
                  width: { size: 100, type: WidthType.PERCENTAGE },
                  children: [new Paragraph({ children: [new ImageRun({ type: 'png', data: imgBuf2, transformation: { width: fit2.width, height: fit2.height } })], alignment: AlignmentType.CENTER })]
                })
              ]
            })
          ];
            ...
            sections: [
              {
                ...
                children: [
                  new Paragraph({ text: `${projectData.title} (第1页 · 基本信息与水源图)`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 200 } }),
                  new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: tableRows1 }),
                  new Paragraph({ text: '制卡单位：杭州市消防救援支队萧山区大队    核准：战训科', alignment: AlignmentType.RIGHT, spacing: { before: 200 } })
                ]
              },
              {
                properties: { page: { size: { orientation: 'landscape' } } },
                children: [
                  new Paragraph({ text: `${projectData.title} (第2页 · 单独总平面图)`, heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 200 } }),
                  new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: tableRows2 }),
                  new Paragraph({ text: '制卡单位：杭州市消防救援支队萧山区大队    核准：战训科', alignment: AlignmentType.RIGHT, spacing: { before: 200 } })
                ]
              }
            ]
          ...
          a.click();
          a.remove();
          showToast('Word 文档已生成（含双页地图截图），请查收！');
        } catch (err) {
